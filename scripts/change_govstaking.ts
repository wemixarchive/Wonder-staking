...
export async function changeStaking() {
    ...
    const gov = await ethers.getContractAt("contracts/interfaces/IGov.sol:IGov", await registry.getContractAddress(B322S("GovernanceContract")));

    let govMems = [];
    for (let i = 0; i < (await gov.getMemberLength()); i++) {
        let ww = fs.readFileSync('./keystore/acct.0'+(i+1)).toString();
